...
class leaky_integrator_VAE_worms(Module):
    def __init__(self, connectome, dt, hidden_init=None, hidden_init_trainable=False,  bias_init=None, tau_init=None, nonlinearity=F.relu, is_Training = True):
        super(leaky_integrator_VAE_worms, self).__init__()
        self.dt = dt
        self.n = connectome.n
        ...
        '''
        '''
        # connectome matrix shuffle along row
        magnitudes_c = connectome.magnitudes_c
        np.random.seed(1)
        np.random.shuffle(magnitudes_c)
        magnitudes_e = connectome.magnitudes_e
        np.random.seed(1)
        np.random.shuffle(magnitudes_e)
        
        sparsity_c = connectome.sparsity_c
        np.random.seed(1)
        np.random.shuffle(sparsity_c)
        sparsity_e = connectome.sparsity_e
        np.random.seed(1)
        np.random.shuffle(sparsity_e)
        '''
        ###
        #pdb.set_trace()
        # fully-connected connectome
        '''
        magnitudes_c = torch.mean(connectome.magnitudes_c) * torch.ones_like(connectome.magnitudes_c)
        magnitudes_e = torch.mean(connectome.magnitudes_e) * torch.ones_like(connectome.magnitudes_e)
        sparsity_c = torch.ones_like(connectome.sparsity_c)
        sparsity_e = torch.ones_like(connectome.sparsity_e)
        '''
        ###
        ...
        #no sign constraints, randomly initialized
        self.magnitudes_c = Parameter(torch.FloatTensor(self.n,self.n).uniform_(-0.01, 0.01))
        self.magnitudes_e = Parameter(torch.FloatTensor(self.n,self.n).uniform_(0, 0.01))
