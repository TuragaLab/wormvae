...
class ELBO_loss(torch.nn.Module):
    ...
    def __init__(self, N, window_size, R):
        ...
        self.mu_neuron_volatge_init = Parameter(torch.rand(N,1))
        self.logvar_neuron_volatge_init = Parameter(0.1 * torch.rand(N,1))
        
        self.logvar_fluorescence_trace_prob = Parameter(torch.rand(N,window_size * R))
        self.logvar_neuron_voltage_prob = Parameter(0.1 * torch.rand(N,window_size * R))
        
    def forward(self, fluorescence_trace_target, missing_fluorescence_target, downsample_factor, mu_neuron_voltage_prob, mu_fluorescence_trace_prob, mu_neuron_voltage_latent, logvar_neuron_voltage_latent, sample_neuron_voltage_latent, recon_weight = 1, KLD_weight = 1):
        ...
        recon_log_likelihood = -torch.mean((torch.distributions.normal.Normal(mu_fluorescence_trace_prob[:,::downsample_factor],std_fluorescence_trace_prob[:,::downsample_factor]).log_prob(fluorescence_trace_target))* (1 - missing_fluorescence_target))/(1 - missing_ratio)
        
        # trajectory probability distrbution: log_likelihood
        # log(P({X}|{S})), scalar average on (N, T * R)
        # std_neuron_volatge_init = torch.exp(0.5 * self.logvar_neuron_volatge_init)
        # trajectory_log_likelihood = -torch.mean((torch.sum(torch.distributions.normal.Normal(mu_neuron_voltage_prob[:,1:],std_neuron_voltage_prob[:,1:]).log_prob(sample_neuron_voltage_latent[:,1:]),1) + torch.distributions.normal.Normal(self.mu_neuron_volatge_init,std_neuron_volatge_init).log_prob(sample_neuron_voltage_latent[:,0]))/mu_neuron_voltage_prob.shape[1])
        
        # KL divergence: KL(q({X}|{F},{S})||Pn), Pn is a learned prior instead of N(0,1)
        # Kingma and Welling. Auto-Encoding Variational Bayes. ICLR, 2014
        # https://arxiv.org/abs/1312.6114
        # closed form:
        KLD_latent = 0.5 * torch.mean((torch.sum(self.logvar_neuron_voltage_prob[:,1:] - logvar_neuron_voltage_latent[:,1:] + (logvar_neuron_voltage_latent[:,1:].exp() + ((mu_neuron_voltage_latent[:,1:] - mu_neuron_voltage_prob[:,1:]).pow(2)))/(self.logvar_neuron_voltage_prob[:,1:].exp()) - 1,1) + self.logvar_neuron_volatge_init - logvar_neuron_voltage_latent[:,0] + (logvar_neuron_voltage_latent[:,0].exp() + ((mu_neuron_voltage_latent[:,0] - self.mu_neuron_volatge_init).pow(2)))/(self.logvar_neuron_volatge_init.exp()) - 1)/mu_neuron_voltage_prob.shape[1])
