...
class ELBO_loss(torch.nn.Module):
    ...
    def __init__(self, N, window_size, R):
        ...
        self.logvar_neuron_voltage_init_0 = Parameter(2*torch.rand(N,1)-1)
        
        self.logvar_fluorescence_trace_prob = Parameter(torch.rand(N,window_size * R))
        self.logvar_neuron_voltage_prob_0 = Parameter(0.1*torch.rand(N,window_size * R))
    ...
    def forward(self, fluorescence_trace_target, missing_fluorescence_target, downsample_factor, mu_neuron_voltage_prob, mu_fluorescence_trace_prob, mu_neuron_voltage_latent, logvar_neuron_voltage_latent, sample_neuron_voltage_latent, recon_weight = 1, KLD_weight = 1, variance_weight = 1, std_reg_weight = 1):
        ...
        std_neuron_voltage_prob = F.softplus(self.logvar_neuron_voltage_prob_0)
        
        std_neuron_voltage_init = F.softplus(self.logvar_neuron_voltage_init_0)
        
        #std_neuron_voltage_init = self.logvar_neuron_voltage_init_0
        #VAE
        ...
        '''
        # no minimization of std term, prior == posterior
        self.logvar_neuron_voltage_init = logvar_neuron_voltage_latent[:,:,0]
        self.logvar_neuron_voltage_prob = logvar_neuron_voltage_latent[:,:,1:]
        '''
        ...
        # no minimization of std term, prior == posterior
        # or AE
        KLD_latent = 0.5 * torch.mean((torch.sum(self.logvar_neuron_voltage_prob - logvar_neuron_voltage_latent[:,:,1:] + (logvar_neuron_voltage_latent[:,:,1:].exp() + ((mu_neuron_voltage_latent[:,:,1:] - mu_neuron_voltage_prob[:,:,1:]).pow(2)))/(self.logvar_neuron_voltage_prob.exp()) - 1,2) + self.logvar_neuron_voltage_init - logvar_neuron_voltage_latent[:,:,0] + (logvar_neuron_voltage_latent[:,:,0].exp() + ((mu_neuron_voltage_latent[:,:,0] - self.mu_neuron_voltage_init[None,:]).pow(2)))/(self.logvar_neuron_voltage_init.exp()) - 1)/mu_neuron_voltage_prob.shape[2])
        '''
        '''
        # no init prior and posterior in KL
        KLD_latent = 0.5 * torch.mean(torch.sum(self.logvar_neuron_voltage_prob[None,:,1:] - logvar_neuron_voltage_latent[:,:,1:] + (logvar_neuron_voltage_latent[:,:,1:].exp() + ((mu_neuron_voltage_latent[:,:,1:] - mu_neuron_voltage_prob[:,:,1:]).pow(2)))/(self.logvar_neuron_voltage_prob[None,:,1:].exp()) - 1,2)/(mu_neuron_voltage_prob.shape[2]-1))
        '''

        #reg_voltage_prior_std = torch.mean(std_neuron_voltage_prob**2)
        
        #with regularization
        #loss = recon_weight * recon_log_likelihood + KLD_weight * KLD_latent + std_reg_weight * reg_voltage_prior_std
        ...
        #return loss, recon_log_likelihood, KLD_latent, reg_voltage_prior_std   #scalar
