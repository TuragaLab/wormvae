...
class ConnectomeData:
    ...
    def make_full_connectome_activity(self, activity_data, activity_neuron_list, ablation_list = []):
        '''
        Takes partial neural activity data (T , N_s) and maps it into a tensor (T,N) where N includes all neurons in this connectome.
        All missing values are NaN. For this connectome N = 302. 

        Parameters:
            activity_data: tensor of calcium traces from a single worm (T,N_s)
            activity_neuron_list: list of neuron labels associated with calcium traces

        Return:
            all_activity: tensor (T, N)
        '''   
        ...
        for i in range(activity_data.shape[1]):
            if activity_neuron_list[i] not in ablation_list:
                ...
                all_activity[:,mask_neuron_index,:] = activity_data[:,i,:] # fill in neural activity to full neuron (T x 302) activity vector
        return all_activity
