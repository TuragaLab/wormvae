...
class Worm_Sensory_Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        #return self.linear2(self.nonlinearity(self.linear1(x.permute(0,2,1))))
        # output: (batch, T, n)
...
class Worm_Inference_Network(nn.Module):
    ...
    def forward(self, fluorescence_trace_target, missing_target_mask, sensory_input):
        ...
        std_neuron_voltage_latent = 0.0 * F.softplus(self.conv5(merge))
...
class WormNetCalcium_deterministic(nn.Module):
    ...
    def __init__(self, connectome, window_size, initialization, R, signs_c, encoder = nn.Linear, inference_network = nn.Linear, device = 'cuda', dt = 0.2, nonlinearity = F.relu, is_Training = True):
        super(WormNetCalcium_deterministic, self).__init__()
        self.dt = dt
        ...
        self.network_dynamics = cnctmnn.leaky_integrator_VAE_worms_deterministic(connectome = self.connectome,
                                                                   dt = dt,
    ...
    def forward(self, fluorescence_full_target, missing_target_mask, odor_input, hidden_init = None):
        ...
        mu_neuron_voltage_prob_deterministic, recurrent_in_deterministic = self.network_dynamics.forward(sensory_input, hidden_init, T_steps = 10)
        # mu_neuron_voltage_prob: (batch, n, T * R)
        mu_neuron_voltage_prob, recurrent_in = self.network_dynamics.stochatics_forward(sensory_input, hidden_init)
        ...
        calcium_activation_deterministic = self.voltage_to_calcium_activation_filter.forward(mu_neuron_voltage_prob_deterministic)
        ...
        mu_calcium_prob_deterministic = self.SCF_exponential(calcium_activation_deterministic/calcium_tau[:,:,None], init_calcium, calcium_tau)
        ...
        mu_fluorescence_trace_prob_deterministic = self.calcium_to_fluorescence_filter.forward(mu_calcium_prob_deterministic)
        
        return mu_neuron_voltage_prob, mu_neuron_voltage_prob_deterministic, mu_fluorescence_trace_prob, mu_fluorescence_trace_prob_deterministic, mu_neuron_voltage_latent, logvar_neuron_voltage_latent, sample_neuron_voltage_latent, std_neuron_voltage_latent_train, calcium_activation, calcium_activation_deterministic, mu_calcium_prob, mu_calcium_prob_deterministic, recurrent_in, recurrent_in_deterministic, sensory_input
